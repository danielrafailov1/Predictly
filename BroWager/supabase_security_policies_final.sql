-- BroWager Database Security Policies (FINAL CORRECTED VERSION)
-- This script enables RLS on all tables and creates appropriate security policies
-- All type casting issues between UUID and text have been resolved
...
CREATE POLICY "Users can update party invites" ON "Party Invites"
    FOR UPDATE USING (auth.uid() = inviter_user_id OR auth.uid() = invitee_user_id);
